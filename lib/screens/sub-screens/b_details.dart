...
class BDetails extends StatefulWidget {
  ...
  final img2;
  final img3;
  BDetails({required this.image, required this.name, required this.district, required this.desc, required this.img1, required this.img2, required this.img3});
...
Future bookNow(name,slottime,slotno,context,image) async{
  ...
  print(currentTime);
  await FirebaseFirestore.instance.collection('BookingsHotel').add({
    'Place': '${name}',
    'UserID': '${uid}',
    'Email': '${uemail}',
  ...
  // var collection1 = FirebaseFirestore.instance.collection('Hotel');
  // var docSnapshot1 = await collection1.doc('$name').get();
  // var data1 = docSnapshot1.data() as Map;
  // var v = data1?['capacity'];
  // print(v);
  // var s = v - slotno;
  //
  // await FirebaseFirestore.instance.collection('Hotel').doc('$name').update({
  //   'capacity': '$s'
  // });
  Navigator.of(context).pop();
}
